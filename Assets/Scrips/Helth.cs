...
{
    private float xMin = -0.5f, xMax = 0.5f,Frequency = 5.0f;
    private float timeValue = 0.0f;

    void Update()
    {
        // Compute the sin position.
        float xValue = Mathf.Sin(timeValue * Frequency);

        // Now compute the Clamp value.
        float xPos = Mathf.Clamp(xValue, xMin, xMax);

        // Update the position of the cube.
        transform.position = new Vector3(xPos, -4.68f, 0.0f);
    ...
    void OnGUI()
    {
        ...
        Frequency = GUI.HorizontalSlider(new Rect(25, 95, 100, 30), Frequency, -1.0f, +10.0f);
        ...
        GUI.Label(new Rect(135, 80, 150, 30), "Frequency: " + Frequency.ToString("f2"), fontSize);
    }
}